...
def q2_1():
    ...
    print(
        "The number of non-missing ratings in training is: %.0f"
        % np.sum(~np.isnan(Y_train)))

    print(
        "The number of non-missing ratings in validation is: %.0f"
        % np.sum(~np.isnan(Y_valid)))
    
    print(Y_valid[~np.isnan(Y_valid),].shape)
    print(Y_train[~np.isnan(Y_train),].shape)
...
def q2_3():
    ...
    k = 50
    fun_obj_w = CollaborativeFilteringWLoss(lammyZ=7, lammyW=7)
    fun_obj_z = CollaborativeFilteringZLoss(lammyZ=7, lammyW=7)
...
def q2_4():
    ...
    k = 50
    fun_obj_w = CollaborativeFilteringWLoss(lammyZ=0, lammyW=7)
    fun_obj_z = CollaborativeFilteringZLoss(lammyZ=0, lammyW=7)
