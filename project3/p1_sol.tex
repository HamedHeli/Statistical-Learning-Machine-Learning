...
\section*{Important: Submission Format \pts{5}}
    ...
    \ask{We will deduct points if you are not tagging the questions properly on Gradescope.}
...
\begin{answer}
${(Xw-y)}^TV(Xw-y) + \frac{\lambda}{2}{||w||_2^2}$
\end{answer}
...
\begin{answer}
${||Xw - y||}_{\infty} + \frac{1}{2}{||\Lambda w||_1^2}$
\end{answer}
\end{enumerate}
...
\begin{answer}
$\frac{1}{2} \norm{w-v}^2 = \frac{1}{2}(w-v)^T(w-v) = \frac{1}{2}\left(w^Tw - w^Tv - v^Tw + v^Tv\right) = \frac{1}{2}\left(w^Tw - 2w^Tv + v^Tv\right) = \frac{1}{2} w^TIw - w^Tv + v^Tv \\ \\ \nabla f(w) = \nabla [\frac{1}{2} w^TIw] - \nabla[w^Tv] + \nabla[v^Tv] = Iw - v = w - v = 0  \longrightarrow w^* = v
$
\end{answer}
\item $f(w)= \frac{1}{2} \norm{Xw - y}^2 + \frac{1}{2} w^T\Lambda w$ (least squares with weighted regularization).
\begin{answer}
$\frac{1}{2} \norm{Xw - y}^2 + \frac{1}{2} w^T\Lambda w = \frac{1}{2} (Xw-y)^T(Xw-y) + \frac{1}{2}w^T\Lambda w = \frac{1}{2} (Xw)^T(Xw) -(Xw)^Ty+\frac{1}{2}y^Ty + \frac{1}{2}w^T\Lambda w = \frac{1}{2}w^TX^TXw - w^TX^Ty + \frac{1}{2}y^Ty + \frac{1}{2} w^T\Lambda w\\ \\
\nabla f(w) = \nabla[\frac{1}{2}w^TX^TXw] - \nabla[w^TX^Ty] + \nabla[\frac{1}{2}y^Ty] + \nabla[\frac{1}{2} w^T\Lambda w] = X^TX w - X^Ty + \Lambda w = 0 \longrightarrow (XTX + \Lambda)w = X^Ty \longleftarrow w^* =(X^TX + \Lambda)^{-1} X^Ty
$
\end{answer}
...
\begin{answer}
$ \frac{1}{2} \sum_{i=1}^n v_i (w^Tx_i - y_i)^2 + \frac{\lambda}{2}\norm{w-w^{(0)}}^2 = \frac{1}{2} (Xw-y)^TV(Xw-y) + \frac{\lambda}{2} (w-w^{(0)})^T(w-w^{(0)}) = \frac{1}{2} (Xw-y)^TV(Xw-y) + \frac{\lambda}{2} w^Tw - \lambda w^Tw^{(0)} +  \frac{\lambda}{2} w^{(0)T}w^{(0)} \\ \\ 
\nabla f(w) = \nabla[\frac{1}{2} (Xw-y)^TV(Xw-y)] + \nabla [\frac{\lambda}{2} w^Tw] - \nabla [\lambda w^Tw^{(0)}] = X^{T}VXw - X^{T}Vy + \lambda (w-w^{(0)}) \longrightarrow (X^TVX + \lambda I) w = X^TVy + \lambda w^{(0)} \longrightarrow w^* = (X^TVX + \lambda I)^{-1} (X^TVy + \lambda w^{(0)})$  
\end{answer}
\end{enumerate}
...
If you run \verb|python main.py 2|, it will load a one-dimensional regression2
...
\begin{answer}
Following Q\ref{sec:lin-sys}.\ref{item:weighted-shrunk-ls}, for $\lambda =0 \text{ and } w^* = 0$, $w^* = (X^TVX)^{-1} (X^TVy)$
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{a3/figs/weighted_least_squares_outliers.pdf}
\end{figure}
\end{answer}
...
which is smooth but less sensitive to outliers than the squared error. \ask{Derive
 ...
 \begin{answer}
$
f(w) {=} \sum_{i=1}^n  \log\left(\exp(w^Tx_i - y_i) + \exp(y_i - w^Tx_i)\right)  \longrightarrow \nabla f(w) = \sum_{i=1}^n \frac{\exp(w^Tx_i-y_i) - \exp(y_i - w^Tx_i)}{\exp(w^Tx_i-y_i) + \exp(y_i - w^Tx_i)}x_i = X^T \frac{\exp(Xw-y) - \exp(y - Xw)}{\exp(Xw-y) + \exp(y - Xw)}
$
...
\begin{enumerate}
    ...
    \ans{It terminates the algorithm when the gradient norm is less than a tolerance or the number of iterations reaches a maximum}
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{a3/figs/robust_least_squares_gd.pdf}
\end{figure}
...
\begin{answer}
Gradient Descent with line search is more iteration efficient as it reaches the minimum objective function in fewer iterations.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\linewidth]{a3/figs/objective_function_comp.pdf}
    \end{figure}
\end{answer}
...
The $y$-intercept of this data is clearly not zero (it looks like it's closer to $100$),
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{a3/figs/least_squares_yes_bias.pdf}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{a3/figs/polynomial_error_curves.pdf}
\end{figure}
As $p$ increases, the training error \textit{generally} decreases as the model has more ability to "memorize" the data. Validation error also decreases initially but it starts increasing after $p = 5$ which shows overfitting. The unexpected behavior at $p = 50$ could be because of the numerical instability but it does not affect the general trend.   
...
\ans{For $k$-means, the outlier is either assigned to the nearest cluster or form its own cluster. In density-based clustering, the outlier is not clustered, as it does not have the minimum number of neighborhood around it.}

\item Why do need random restarts for $k$-means but not for density-based clustering?

\ans{$k$-means algorithm starts with randomly assigning centroids and then cluster assignment; hence, the cluster assignment is dependent on the initial random assignment. Density-based algorithm though is not sensitive to the initial selection.}

\item Can hierarchical clustering find non-convex clusters?
\ans{Yes; in hierarchical clustering, merging two clusters may result in non-convex clusters.}

\item For model-based outlier detection, list an example method and problem with identifying outliers using this method.
\ans{Using $Z$-score or quartiles is a model-based outlier detection but it fails when the data distribution is not uni-modal.}
...
\ans{Graphical-based outlier detection is challenging when more than two features are involved. Looking at pair-wise scatter plots can solve the problem but it is hard when many features are involved.}

\item For supervised outlier detection, list an example method and problem with identifying outliers using this method.
\ans{If we do have flags for some outliers, supervised learning methods such as GLM, find other outliers. The limitation is, however, a separate step is needed to create the initial flag for outliers using an unsupervised learning method.}
...
\ans{If the objective function's gradient has a closed-form solution, it does not make sense to use gradient descent as the closed-form solution is computationally cheaper. If closed-form solution does not exists, like for the infinity norm example above, the gradient descent would still make sense.}
...
\ans{Adding a column of $1$ values in linear regression causes intercept (bias) which can improve the fit. In decision tree, however, it does not make sense to do the same as as constant feature does not add more information in the split.}
...
\ans{Because it does not have a closed-form solution.}
...
\ans{Too small learning rate results in minimal changes in each iteration and hence the algorithm may not converge to minimum. Too large learning rate also causes the algorithm to pass the minimum (overshoot) and oscillates. }

\item What is the purpose of the log-sum-exp function and how is this related to gradient descent?
\ans{`log-sum-exp` objective function approximates infinity norm. When robust regression with `log-sum-exp` objective function is considered, the solution does not have a closed-form solution and so gradient descent should be used.}

\item What type of non-linear transform might be suitable if we had a periodic function?
\ans{Feature transformations with trigonometric functions, like `sin` and `cos`.}
